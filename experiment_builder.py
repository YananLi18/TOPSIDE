...
from utils.loss import mae, smape, r_square
...
class ExperimentBuilder(object):
    def __init__(self, args, data, model, device):
        ...
        self.saved_models_filepath, self.logs_filepath, self.samples_filepath = build_experiment_folder(experiment_name=self.args.experiment_name,
                                                                                                        metric_name=self.args.label_name,
    ...
    def build_loss_summary_string(self, summary_losses):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "accuracy" in key or "smape" in key or "mae" in key:
                value = float(value)
    ...
    def col_test_evaluation_iteration(self, val_sample, model_idx, sample_idx, per_model_per_batch_preds, per_model_per_batch_support_preds, pbar_test, encoder):
        """
        Runs a validation iteration, updates the progress bar and returns the total and current epoch val losses.
        :param val_sample: A sample from the data provider
        :param total_losses: The current total losses dictionary to be updated.
        ...
        data_batch = (
            x_support_set, x_target_set, y_support_set, y_target_set)
        if self.args.pretrain_model == 'meta_learning':
            start = time.time()
            losses, per_task_preds, per_task_support_preds = self.model.col_run_validation_iter(data_batch=data_batch)
            end = time.time()
        else:
            losses, _, _ = self.model.col_run_validation_iter(data_batch=data_batch)
            loss_item, per_task_preds, per_task_support_preds = pretrain_col_run_validation_iter(data_batch=data_batch, pretrain_model=encoder, device=self.device)
            losses['loss'], losses['accuracy'] = loss_item, -1*loss_item.item()
        ...
        if self.args.start_idx == 0:
            ...
            new_per_task_preds, new_per_task_support_preds = per_task_preds, per_task_support_preds
        per_model_per_batch_preds[model_idx].extend(list(new_per_task_preds))
        per_model_per_batch_support_preds[model_idx].extend(list(new_per_task_support_preds))
        ...
        return per_model_per_batch_preds, per_model_per_batch_support_preds, end-start
    ...
    def collaborative_evaluated_test_set(self, predictor, encoder=None):
        ...
        # Step 1: Replace NaN values with a very low value
        val_acc = np.nan_to_num(val_acc, nan=np.nanmin(val_acc) - 1)
        sorted_idx = np.argsort(val_acc, axis=0).astype(dtype=np.int32)[::-1][:self.args.top_n_models]
        ...
        top_n_idx = val_idx[:self.args.top_n_models]
        per_model_per_batch_preds = [[] for i in range(self.args.top_n_models)]
        per_model_per_batch_targets = [[] for i in range(self.args.top_n_models)]
        ...
        if self.args.dataset_name == 'sneset_dataset':
            ...
            # num_output_classes = num_output_classes if self.args.loss_name == 'SupCR' else num_output_classes * 2
            ...
            for x_support, y_support, x_target, y_target in zip(per_batch_support_preds, per_batch_support_targets, per_batch_preds, per_batch_targets):
                y_target = y_target.reshape(-1)
                start1 = time.time()
                predictor.fit(x_support, y_support)
                end1 = time.time()
                test_pred = predictor.predict(x_target).reshape(-1)
                end2 = time.time()
                ...
                infer_time.append(end2 - end1)
                mae_lst.append(pred_mae)
                smape_lst.append(pred_smape)
                r2_lst.append(pred_r2)
    ...
    def collaborative_nn_evaluated_test_set(self, encoder):
        train_rng = np.random.RandomState(seed=self.args.train_seed)
        train_seed = train_rng.randint(1, 999999)
        test_rng = np.random.RandomState(seed=self.args.val_seed)
        ...
        if self.args.dataset_name == 'sneset_dataset':
            ...
            mae_lst, smape_lst, r2_lst = [], [], []
            start_time = time.time()
            for i, (x_support, y_support, x_target, y_target) in enumerate(zip(per_batch_support_preds, per_batch_support_targets, per_batch_preds, per_batch_targets)):
                # y_target = y_target.reshape(-1)
                train_time, infer_time, pred_mae, pred_smape, pred_r2 = nn_pipeline(self.args, x_support, y_support, x_target, y_target, dic_num, dic_idx)
                ...
                r2_lst.append(pred_r2)
                if i % 10 == 0:
                    print(f"{time.time()-start_time} pass, {i} epochs have been processed.")
    ...
    def evaluated_test_set_using_the_best_models(self, top_n_models):
        ...
        # per_epoch_statistics = pd.read_csv(self.logs_filepath + "/summary_statistics.csv")
        # val_acc = per_epoch_statistics['val_accuracy_mean'].values
        val_idx = np.array([i for i in range(len(val_acc))])
        ...
        per_batch_preds = np.mean(per_model_per_batch_preds, axis=0)
        if self.args.dataset_name == 'sneset_dataset':
            per_batch_preds = per_batch_preds.reshape(self.args.num_evaluation_tasks,
            ...
            loss_lst, acc_lst, smape_lst = [], [], []
            for i in range(self.args.num_evaluation_tasks):
                preds = per_batch_preds[i, :]
                targets = per_batch_targets[i, :]
                ...
                numerator = np.abs(preds - targets)
                denominator = 0.5 * (preds + targets)

                loss_lst.append(np.mean(numerator))
                smape_lst.append(np.mean(numerator / denominator))
                acc_lst.append(R_squared)
            accuracy = np.mean(acc_lst)
            accuracy_std = np.std(acc_lst)

            mae, mae_std = np.mean(loss_lst), np.std(loss_lst)
            smape, smape_std = np.mean(smape_lst), np.std(smape_lst)
            test_losses = {
                "test_mae_mean": mae, "test_mae_std": mae_std,
                "test_smape_mean": smape, "test_smape_std": smape_std,
                "test_accuracy_mean": accuracy, "test_accuracy_std": accuracy_std}
        else:
    ...
    def self_evaluated_test_set_using_the_best_models(self, top_n_models, num_epochs):
        '''
        For each task, training with the support set (with specific epochs) and
        evaluation with target set.
        '''
        per_epoch_statistics = self.state['per_epoch_statistics']
        val_acc = np.copy(per_epoch_statistics['val_accuracy_mean'])
        # per_epoch_statistics = pd.read_csv(self.logs_filepath + "/summary_statistics.csv")
        ...
        for idx, model_idx in enumerate(top_n_idx):
            ...
            with tqdm.tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_test:
                for sample_idx, test_sample in enumerate(
                        self.data.get_test_batches(total_batches=int(self.args.num_evaluation_tasks / self.args.batch_size),
                                                   augment_images=False)):
                    # shape of data torch.Size([8, 5, 64, 16, 1, 1]) torch.Size([8, 5, 64, 16, 1, 1])
                    # torch.Size([8, 5, 64]) torch.Size([8, 5, 64])
    ...
    def run_experiment(self):
        ...
        """
        with tqdm(initial=self.state['current_iter'], total=int(self.args.total_iter_per_epoch * self.args.total_epochs)) as pbar_train:
            while (self.state['current_iter'] < (self.args.total_epochs * self.args.total_iter_per_epoch)) and (self.args.evaluate_on_test_set_only == False):
                for train_sample_idx, train_sample in enumerate(
                    ...
                    if self.state['current_iter'] % self.args.total_iter_per_epoch == 0:
                        ...
                        total_losses = {}
                        val_losses = {}
                        with tqdm(total=int(self.args.num_evaluation_tasks / self.args.batch_size)) as pbar_val:
                        ...
                        if self.epochs_done_in_this_run >= self.total_epochs_before_pause:
                            print("train_seed {}, val_seed: {}, at pause time".format(self.data.dataset.seed["train"],
                                                                                      self.data.dataset.seed["val"]))
                            # sys.exit()
            if self.args.num_output_classes == 1:

...
class MetaLinearXLayer(nn.Module):
    def __init__(self, input_shape, num_filters, use_bias):
        ...
        learning setting.
        :param input_shape: The shape of the input data, in the form (b, 2f)
        :param num_filters: Number of output filters
        :param use_bias: Whether to use biases or not.
        """
        super(MetaLinearXLayer, self).__init__()
        b, c = input_shape

        self.output_shape = int(c/2)

        self.use_bias = False
        self.weights = nn.Parameter(torch.tensor([1, 0, 0], dtype=torch.float))
        if self.use_bias:
            self.bias = nn.Parameter(torch.zeros(num_filters))
    ...
    def forward(self, x, params=None):
        ...
        :param x: Input data batch, in the form (b, 2f)
        ...
        :return: Output data batch, in the form (b, f).
        """
        if params is not None:
            params = extract_top_level_dict(current_dict=params)
            if self.use_bias:
            ...
            weight = self.weights
            bias = None
        x_left, x_right = x.split(self.output_shape, dim=1)
        w0, w1, w2 = F.softmax(weight)
        return w0 * x_right + w1 * torch.tanh(x_left) * x_right + w2 * torch.sigmoid(x_left) * x_right
...
class MetaGraphConvolution(nn.Module):
    def __init__(self, input_shape, num_filters, use_bias=True, use_adp=True):
        super(MetaGraphConvolution, self).__init__()

        self.b, self.num_nodes = input_shape
        self.in_features = 1
        self.time = True
        self.out_features = num_filters
        self.weights = nn.Parameter(torch.FloatTensor(self.in_features, num_filters))

        self.final_weights = nn.Parameter(torch.FloatTensor(1, self.num_nodes))

        self.use_adp = use_adp
        if self.use_adp:
            self.nodevec1 = nn.Parameter(torch.FloatTensor(self.num_nodes, 10), requires_grad=True)
            self.nodevec2 = nn.Parameter(torch.FloatTensor(10, self.num_nodes), requires_grad=True)
            nn.init.xavier_uniform_(self.nodevec1)
            nn.init.xavier_uniform_(self.nodevec2)
        else:
            self.adj = nn.Parameter(torch.FloatTensor(
                self.num_nodes, self.num_nodes).fill_(1/self.num_nodes), requires_grad=True)
            nn.init.xavier_uniform_(self.adj)
        nn.init.xavier_uniform_(self.weights)
        nn.init.xavier_uniform_(self.final_weights)
    ...
    def forward(self, input, params=None):
        ...
        if params is not None:
            params = extract_top_level_dict(current_dict=params)
            if self.use_bias:
                (weights, final_weights, bias) = params["weights"], params["final_weights"], params["bias"]
                if self.use_adp:
                    (nodevec1, nodevec2) = params["nodevec1"], params["nodevec2"]
                else:
                    adj = params["adj"]
            else:
                (weights, final_weights) = params["weights"], params["final_weights"]
                if self.use_adp:
                    (nodevec1, nodevec2) = params["nodevec1"], params["nodevec2"]
                else:
                    adj = params["adj"]
            ...
            weights, final_weights, bias = self.weights, self.final_weights, self.bias
            if self.use_adp:
                nodevec1, nodevec2 = self.nodevec1, self.nodevec2
            else:
                adj = self.adj
        else:
            weights, final_weights = self.weights, self.final_weights
            if self.use_adp:
                nodevec1, nodevec2 = self.nodevec1, self.nodevec2
            else:
                adj = self.adj
        ...
        if self.use_adp:
            ...
            if self.time:
                ...
                eye = torch.eye(self.num_nodes).to(device)
            adj = adp + eye
            degree = torch.diag(torch.pow(adj.sum(1), -0.5))
            adp = degree.mm(adj).mm(degree)
        else:
            if self.time:
                eye = torch.eye(self.num_nodes)
                self.time = False
        ...
        support = torch.einsum('bnd,df->bnf', input.unsqueeze(-1), weights)
        output = torch.einsum('nd,bdm->bnm', adp, support)
        if self.use_bias is not None:
            output = output + bias  # output: [B, N, D_out]
        output_swapped = output.transpose(-1, -2).contiguous() # output: [B, D_out, N]
        return torch.squeeze(F.linear(input=output_swapped, weight=final_weights, bias=None))
...
class CausalEncoder(nn.Module):
    def __init__(self, im_shape, num_output_classes, args, device, meta_classifier=True, normalization=True,
                 no_bn_learnable_params=False,):
        """
        Builds a multilayer perceptron network. It also provides functionality for passing external parameters to be
        used at inference time. Enables inner loop optimization readily.
        :param im_shape: The input batch shape [# b, # of QoS, 1, 1].
        :param num_output_classes: The number of output classes/dimesions of the network.
        :param args: A named tuple containing the system's hyperparameters.
        :param device: The device to run this on.
        :param meta_classifier: A flag indicating whether the system's meta-learning (inner-loop) functionalities should
        be enabled.
        ...
        b, c, self.h, self.w = im_shape  # b, # of QoS, 1, 1
        self.normalization = normalization
        self.use_per_step_bn_statistics = args.per_step_bn_statistics
        self.meta_layer = meta_classifier
        self.no_bn_learnable_params = no_bn_learnable_params
        ...
        self.u2_dims = 2 ** (self.pow_dim - 1)
        self.u3_dims = 2 ** (self.pow_dim - 2)
        self.u4_dims = 2 ** (self.pow_dim - 3)

        # 76 - 64 / 128
        self.all_dim = self.o1_dims + self.o2_dims + self.o3_dims + self.o4_dims + \
                       self.u1_dims + self.u2_dims + self.u3_dims + self.u4_dims
        self.emb_dim = num_output_classes
    ...
    def build_network(self):
        ...
        self.layer_dict['u1_en'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o1_dims + self.o2_dims,),
                                                   num_filters=self.u1_dims, use_bias=True)
        self.layer_dict['u2_en'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o2_dims + self.o3_dims,),
                                                   num_filters=self.u2_dims, use_bias=True)
        self.layer_dict['u3_en'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o3_dims + self.o4_dims,),
                                                   num_filters=self.u3_dims, use_bias=True)

        self.layer_dict['u2_de'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o1_dims + self.u1_dims,),
                                                   num_filters=self.u2_dims, use_bias=True)
        self.layer_dict['u2_f'] = MetaLinearXLayer(input_shape=(out.shape[0], 2 * self.u2_dims),
                                                  num_filters=self.u2_dims, use_bias=True)

        self.layer_dict['u3_de'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o2_dims + self.u2_dims,),
                                                   num_filters=self.u3_dims, use_bias=True)
        self.layer_dict['u3_f'] = MetaLinearXLayer(input_shape=(out.shape[0], 2 * self.u3_dims),
                                                  num_filters=self.u3_dims, use_bias=True)

        self.layer_dict['u4_de'] = MetaGraphConvolution(input_shape=(out.shape[0], self.o3_dims + self.u3_dims,),
                                                   num_filters=self.u4_dims, use_bias=True)

        u1 = self.layer_dict['u1_en'](out[:, :self.o1_dims + self.o2_dims].float())

        u2_en = self.layer_dict['u2_en'](out[:, self.o1_dims:self.o1_dims + self.o2_dims + self.o3_dims].float())
        u2_de = self.layer_dict['u2_de'](torch.cat((out[:, :self.o1_dims].float(), u1), 1))
        u2 = self.layer_dict['u2_f'](torch.cat((u2_en, u2_de), 1))

        u3_en = self.layer_dict['u3_en'](out[:, self.o1_dims + self.o2_dims:self.o1_dims + self.o2_dims + self.o3_dims + self.o4_dims].float())
        u3_de = self.layer_dict['u3_de'](torch.cat((out[:, self.o1_dims:self.o1_dims + self.o2_dims].float(), u2), 1))
        u3 = self.layer_dict['u3_f'](torch.cat((u3_en, u3_de), 1))

        u4 = self.layer_dict['u4_de'](torch.cat((out[:, self.o1_dims + self.o2_dims:self.o1_dims + self.o2_dims + self.o3_dims].float(), u3), 1))

        self.layer_dict['linear'] = MetaLinearLayer(input_shape=(out.shape[0], self.all_dim),
                                                    num_filters=self.emb_dim, use_bias=True)

        # self.layer_dict['linear'] = MetaLinearLayer(input_shape=(out.shape[0], out.shape[1]),
        #                                             num_filters=self.emb_dim, use_bias=True)

        out = self.layer_dict['linear'](torch.cat((out.float(), u1, u2, u3, u4), 1))
        # out = self.layer_dict['linear'](out.float())
        print("CausalEncoder build", out.shape)
    ...
    def forward(self, x, num_step, params=None, training=False, backup_running_statistics=False):
        ...
        out = out.view(out.size(0), -1)
        u1 = self.layer_dict['u1_en'](out[:, :self.o1_dims + self.o2_dims].float(), param_dict['u1_en'])

        u2_en = self.layer_dict['u2_en'](out[:, self.o1_dims:self.o1_dims + self.o2_dims + self.o3_dims].float(), param_dict['u2_en'])
        u2_de = self.layer_dict['u2_de'](torch.cat((out[:, :self.o1_dims].float(), u1), 1), param_dict['u2_de'])
        u2 = self.layer_dict['u2_f'](torch.cat((u2_en, u2_de), 1), param_dict['u2_f'])

        u3_en = self.layer_dict['u3_en'](
            out[:, self.o1_dims + self.o2_dims:self.o1_dims + self.o2_dims + self.o3_dims + self.o4_dims].float(), param_dict['u3_en'])
        u3_de = self.layer_dict['u3_de'](torch.cat((out[:, self.o1_dims:self.o1_dims + self.o2_dims].float(), u2), 1), param_dict['u3_de'])
        u3 = self.layer_dict['u3_f'](torch.cat((u3_en, u3_de), 1), param_dict['u3_f'])

        u4 = self.layer_dict['u4_de'](
            torch.cat((out[:, self.o1_dims + self.o2_dims:self.o1_dims + self.o2_dims + self.o3_dims].float(), u3), 1), param_dict['u4_de'])

        out = self.layer_dict['linear'](torch.cat((out.float(), u1, u2, u3, u4), 1), param_dict['linear'])
        # out = self.layer_dict['linear'](out.float(), param_dict['linear'])

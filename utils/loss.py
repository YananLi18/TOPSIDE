...
class FeatureSimilarity(nn.Module):
    ...
    def forward(self, features):
        ...
        if self.similarity_type == 'l2':
            return - (features[:, None, :] - features[None, :, :] + 1e-4).norm(2, dim=-1)
        else:
            raise ValueError(self.similarity_type)


class RnCLoss(nn.Module):
    def __init__(self, temperature=2, label_diff='l1', feature_sim='l2', epsilon=1e-6):
    ...
    def forward(self, features, labels):
        ...
        for k in range(n - 1):
            ...
            tmp = (neg_mask * exp_logits).sum(dim=-1)

            pos_log_probs = pos_logits - torch.log(tmp + 1e-3) # 2bs

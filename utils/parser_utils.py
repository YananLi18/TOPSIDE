...
def get_args():
    ...
    parser.add_argument('--image_height', nargs="?", type=int, default=1)
    parser.add_argument('--image_width', nargs="?", type=int, default=1)
    parser.add_argument('--image_channels', nargs="?", type=int, default=16)
    ...
    parser.add_argument('--label_name', type=str, default="buffer_rate")
    parser.add_argument('--loss_name', type=str, default="SupCR")
    parser.add_argument('--work_path', type=str, default="/data2/lyn/www24/HTTYMP/", help='work path')
    parser.add_argument('--corruption_num', type=int, default=0, help='the number to apply data corruption')
    parser.add_argument('--top_n_models', type=int, default=5)
    parser.add_argument('--start_idx', type=int, default=4, help='whether contain categories')
    parser.add_argument('--num_of_classes', type=int, default=4, help='whether contain categories')
    parser.add_argument('--pretrain_model', type=str, default="meta_learning")
    parser.add_argument('--bnn_collaborative', action='store_true', help='whether are nn collaborative')
...
def get_encoder(args, device):
    # device = torch.cuda.current_device()
    if args.pretrain_model != 'meta_learning':
        torch.hub.set_dir('/data2/lyn/www24/HTTYMP/baselines/collaborative/cache')
        if args.pretrain_model == 'resnet50':
            encoder = torchvision.models.resnet50(pretrained=True)
        elif args.pretrain_model == 'alexnet':
            encoder = torchvision.models.alexnet(pretrained=True)
        elif args.pretrain_model == 'googlenet':
            encoder = torchvision.models.googlenet(pretrained=True)
        elif args.pretrain_model == 'vgg16':

...
for test_sample_idx, test_sample in enumerate(MLDataLoader.get_test_batches(total_batches=int(args.num_evaluation_tasks / args.batch_size),
    ...
    y_support_set, y_target_set = y_support_set.view(-1, sn, 1), y_target_set.view(-1, args.num_target_samples) # y_support_set in shape [40, 64, 1];
    ...
    for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(
        ...
        start1 = time()
        pipe_best.fit(x_support, y_support)
        end1 = time()
        test_pred = pipe_best.predict(x_target).reshape(-1)
        end2 = time()

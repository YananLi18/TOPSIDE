...
for test_sample_idx, test_sample in enumerate(MLDataLoader.get_test_batches(total_batches=int(args.num_evaluation_tasks / args.batch_size),
    ...
    x_support_set, x_target_set = torch.squeeze(x_support_set).view(-1, sn, c), torch.squeeze(x_target_set).view(-1, args.num_target_samples, c) # x_support_set in shape [40, 64, 16];
    y_support_set, y_target_set = y_support_set.view(-1, sn, 1), y_target_set.view(-1, args.num_target_samples, 1) # y_support_set in shape [40, 64, 1];
    ...
    for task_id, (x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in enumerate(
        ...
        y_support, y_target = y_support_set_task.numpy(), y_target_set_task.numpy() # y_support in shape [320, 1]; y_target in shape [320, ];
...
print(f"Training time is {np.mean(train_time_task):.2f} s")
print(f"Inference time is {np.mean(infer_time_task):.2f} s")

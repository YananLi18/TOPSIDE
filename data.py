...
class FewShotLearningRegressionParallel(Dataset):
    ...
    def load_dataset(self):
        ...
        total_samples = self.num_samples_per_class + self.args.num_target_samples
        ...
        x_val_classes = label_list[x_train_id: x_val_id]
        x_test_classes = label_list[x_val_id:x_test_id]

        x_train_df = dataset_df[dataset_df['node_name'].isin(x_train_classes)].copy()
        x_val_df = dataset_df[dataset_df['node_name'].isin(x_val_classes)].copy()
        ...
        if self.args.start_idx == 0:
            ...
            for f in ['node_name', 'NbClients','DASHPolicy','StallLabel', ]:
                ...
                x_train_df[f] = x_train_df[f].map(dic1)
                x_val_df[f] = x_val_df[f].map(dic1)
                x_test_df[f] = x_test_df[f].map(dic1)

        x_train, x_val, x_test = {}, {}, {}
        for label in x_train_classes:
            tmp = x_train_df.sample(n=total_samples, random_state=self.seed['train']).values[:, self.args.start_idx:]
            tmp = np.expand_dims(np.expand_dims(tmp, 1), 2)
            x_train[label] = tmp
        for label in x_val_classes:
            tmp = x_val_df.sample(n=total_samples, random_state=self.seed['val']).values[:, self.args.start_idx:]
            tmp = np.expand_dims(np.expand_dims(tmp, 1), 2)
            x_val[label] = tmp
        for label in x_test_classes:
            tmp = x_test_df.sample(n=total_samples, random_state=self.seed['test']).values[:, self.args.start_idx:]
            tmp = np.expand_dims(np.expand_dims(tmp, 1), 2)
            x_test[label] = tmp
    ...
    def get_set(self, dataset_name, seed, augment_images=False):
        ...
        selected_classes = rng.choice(list(self.dataset_size_dict[dataset_name].keys()),
                                      size=self.num_classes_per_set, replace=True)  # num_classes_per_set 需要重新设置
        ...
        for class_entry in selected_classes:
            # num_samples_per_class 和 num_target_samples 重新思考设置多少合适？
            ...
            for sample in choose_samples_list:
                ...
                x_class_data = torch.from_numpy(choose_samples_x).permute(2, 0, 1)

                class_image_samples.append(x_class_data)
                class_labels.append(float(choose_samples_y))
            class_image_samples = torch.stack(class_image_samples)
            x_images.append(class_image_samples)  # [s, h, w, c]
            y_labels.append(class_labels)

        x_images = torch.stack(x_images)  # [task batch, samples, h, w, c]
...
class MetaLearningSystemDataLoader(object):
    def __init__(self, args, current_iter=0):
        ...
        if args.dataset_name == 'sneset_dataset':
            self.dataset = FewShotLearningRegressionParallel(args=args)
        else:
